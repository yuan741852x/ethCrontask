...
func RunPayInMatch() {
	...
	for {
		...
		for _, strt := range recordList {
			...
			if inPool {
				go func(item ScanBlockRecord) {
					result, err := global.Rdb2.HGet(config.WalletPools, item.ToHex).Result()
					if err != nil {
						return
					}
					ethOrder := EthOrder{}
					...
					if ethOrder.State != 3 && ethOrder.AmountFrom == round(int64(item.Value)) {
						redisVal.State = 2
						ethOrder.State = 2
						return
					} else if ethOrder.State != 3 && ethOrder.AmountTo != round(int64(item.Value)) {
						redisVal.State = 8
						ethOrder.State = 8
						ethOrder.Collection = round(int64(item.Value))
